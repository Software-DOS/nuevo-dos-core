...
namespace WebAppConexion.Controllers
{
    ...
    public class GTHEvaluacionController : Controller
    {
        ...
        {
            Console.WriteLine("Raw model: " + JsonConvert.SerializeObject(model));

            try
            {
                // ✅ TU CÓDIGO ORIGINAL PERO CORREGIDO
                ...
                Console.WriteLine($"Mapeando - IdEmpleado: {db.IdEmpleado}, Anio: {db.Anio}");

                Console.WriteLine($"ID Empleado final: {db.IdEmpleado}");
                Console.WriteLine($"Año final: {db.Anio}");

                var responseResult = await _repository.Gestionar(model.Tipo, db);
                return responseResult;
            }
            catch (Exception ex)
            {
                return new { Codigo = -999, Mensaje = $"Error: {ex.Message}" };
            }
        }
        ...
        public async Task<IActionResult> ObtenerPorId(int id)
        {
            try
            {
                ...
                var viewModel = new GTHEvaluacionViewModel
                {
                    ...
                    PlanAccion = evaluacion.PlanAccion,
